...
public class SNodeServiceHandler implements SNodeService.Iface
{
	...
	public String getKey(String input, int range){
		try {
			...
			BigInteger key = no.mod(BigInteger.valueOf(range));
			...
			while(checkKey.contains(tempKey)){
				...
				key = no.mod(BigInteger.valueOf(range));
				tempKeyString = key.toString(10);
				tempKey = Integer.parseInt(tempKeyString);
			}
	...
	public NodeInfo join(String ip, String port){
		...
		if(!joinLock){
			...
			int key = Integer.parseInt(getKey(ip + ":" + port, (int)Math.pow(2, tableSize)));
			checkKey.add(key);
			curNode.nodeKey = key;
			if (allNodes.size() < nodeMax){
				if (allNodes.size() == 0){
				...
				if(allNodes.size() == nodeMax) System.out.println("All nodes have been joined into the DHT, you can do get or set operation in the client node now.");
				else System.out.println("Still need " + (nodeMax - allNodes.size()) + " nodes to finish the DHT.");
			}
			else {
				System.out.println("All nodes have been joined into the DHT now.");
				System.out.println("You cannnot add more node into the DHT.");
				retNode.nodeIp = "too many";
			}
